...
namespace JADI {
    ...
    void Application::Run() {
        JADI_CORE_INFO("Initialized Log!");
        auto last = std::chrono::high_resolution_clock::now();
        long long delta = 0;

        if (!glfwInit()) {
            JADI_CORE_FATAL("Failed to initialize GLFW");
            return;
        }

        GLFWwindow* window = glfwCreateWindow(640, 480, "Hello World", nullptr, nullptr);
        if (!window) {
            JADI_CORE_ERROR("Failed to create GLFW window");
            glfwTerminate();
            return;
        }
        else { JADI_CORE_INFO("GLFW window was created"); }
